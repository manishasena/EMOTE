...
class TestRLAgent:
    def __init__(self, sympathetic_mode, empathetic_mode, Ghost_model_type, loss1_weight = 0.5, bench_mark = 0, training_mode = True, load_file = None):
        ...
        if self.empathetic_mode:
            if self.params['load_file'] is None:
                self.q_net_human.Greedy_model.set_weights(self.q_net_greedy_target.model.get_weights())
        ...
        self.greedy_rewards = np.array([0, 0, 0, 0, 0])
        self.robot_rewards_pred = np.array([0, 0, 0, 0, 0])
    ...
    def human_reward_prediction(self):
        ...
        for i in range(self.params['batch_size']):
            ...
            self.human_rewards = self.human_rewards - (0.02 * reward_vector[i] * difference_R)
...
for loop in loops:
    ...
    for episodes in episode_range:
        ...
        if agents.training_mode:
            ...
            log_file.write("# %4d | steps_t: %5d |r: %5d | cost: %6f | cost_greedy: %6f | cost_human: %6f | human_perc: %6f | e: %10f | robot_foodleft: %2d | H_foodleft: %2d | door_opened: %r | won: %r \n" %
                                    (episodes, agents.cnt, agents.cumulative_reward, float(agents.cost_total)/float(agents.cnt), float(agents.greedy_cost_total)/float(agents.cnt), float(agents.human_cost_total)/float(agents.cnt_human), float(agents.human_perc_correct)/float(agents.cnt_human) , agents.epsilon, len(env.agents[0].foodpos), len(env.agents[1].foodpos), door_was_opened, won))
        else:
        ...
        if agents.training_mode:
            ...
            if agents.sympathetic_mode:
                ...
                reward_array_vector = [float(agents.reward_foodh)/float(agents.beta_foodh_cnt), float(agents.reward_food)/float(agents.beta_food_cnt),float(agents.reward_door)/float(agents.beta_door_cnt),float(agents.reward_win)/float(agents.beta_win_cnt),float(agents.reward_step)/float(agents.beta_step_cnt)]
                log_file.write("# %4d | reward: %r \n" %
            ...
            if agents.sympathetic_mode:
                ...
                reward_array_vector = [float(agents.reward_foodh_pnext)/float(agents.beta_foodh_cnt), float(agents.reward_food_pnext)/float(agents.beta_food_cnt),float(agents.reward_door_pnext)/float(agents.beta_door_cnt),float(agents.reward_win_pnext)/float(agents.beta_win_cnt),float(agents.reward_step_pnext)/float(agents.beta_step_cnt)]
                log_file.write("# %4d | reward: %r \n" %
                                            (episodes,list(reward_array_vector)))
