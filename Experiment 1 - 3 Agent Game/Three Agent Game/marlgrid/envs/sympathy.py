...
class SympathyMultiGrid(MultiGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        # Wall positions
        wall_pos = [] #[[6,1],[6,2],[4,2],[3,2],[2,2],[2,4],[2,5],[2,6],[4,6],[5,6],[3,4],[5,4],[6,0]]
        ...
        door_position = [] #[[7,2]]
        human_specific_food = []#[[7,1]]
        button_position = [] #[[7,3]]

        human_food_positions = []
        while len(human_food_positions) < 10:
            ...
            if not ((x == 3) & (y == 1)):
                if not ((x == 3) & (y == 5)):
                    if not ((x == 4) & (y == 3)):
                        if [x,y] not in human_food_positions:
                            if [x,y] not in wall_pos:
                                if [x,y] not in door_position:
                                    if [x,y] not in button_position:
                                        human_food_positions.append([x,y])

        #if [x,y] not in human_specific_food:
        #    human_food_positions.append([7,1])

        
        adversary_food_positions = []
        
        while len(adversary_food_positions) < 10:
            x = np.random.randint(1,8)
            y = np.random.randint(1,8)
            ...
            if not ((x == 3) & (y == 1)):
                if not ((x == 3) & (y == 5)):
                    if not ((x == 4) & (y == 3)):
                        if [x,y] not in human_food_positions:
                            if [x,y] not in adversary_food_positions:
                                if [x,y] not in wall_pos:
                                    if [x,y] not in door_position:
                                        if [x,y] not in button_position:
                                            adversary_food_positions.append([x,y])
        
        robot_food_positions = []
        while len(robot_food_positions) < 10:
            ...
            if not ((x == 3) & (y == 1)):
                if not ((x == 3) & (y == 5)):
                    if not ((x == 4) & (y == 3)):
                        if [x,y] not in human_food_positions:
                            if [x,y] not in adversary_food_positions:
                                if [x,y] not in robot_food_positions:
                                    if [x,y] not in wall_pos:
                                        if [x,y] not in door_position:
                                            if [x,y] not in button_position:
                                                robot_food_positions.append([x,y])
        ...
        if getattr(self, 'randomize_goal', True):
            ...
            for k in range(len(human_food_positions)):
                ...
                self.try_place_obj(FoodOther(color="yellow", reward=2), pos)
                self.agents[1].foodpos.append(pos)

            for k in range(len(adversary_food_positions)):
                pos = np.array(adversary_food_positions[k]) #self.place_obj(FoodOther(color="orange", reward=0), max_tries=1000)
                self.try_place_obj(FoodAdversary(color="purple", reward=2), pos)
                self.agents[2].foodpos.append(pos)

            for k in range(len(button_position)):
                pos = np.array(button_position[k]) #self.place_obj(FoodOther(color="orange", reward=0), max_tries=1000)
                self.try_place_obj(Food(color="green", reward=-1), pos)
