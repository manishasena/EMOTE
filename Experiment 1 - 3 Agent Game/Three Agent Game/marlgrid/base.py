...
from .objects import WorldObj, Wall, Goal, Lava, GridAgent, BonusTile, BulkObj, COLORS,Food,FoodOther, FoodAdversary, EmptySpace
            ...
            if agent.active and action != 6:
                ...
                # Move forward
                    ...
                    if can_move:
                        ...
                        if (isinstance(fwd_cell, (FoodOther)) or isinstance(fwd_cell, (FoodAdversary))) and (agent_no == 0):
                            agent.eatfood = True
                            agent.fwd_cell_pos = fwd_cell
                        ...
                        if isinstance(fwd_cell, (FoodAdversary)) and (agent_no == 2):
                            agent.eatfood = True
                            agent.fwd_cell_pos = fwd_cell
            ...
            if agent.eatfood:
                ...
                if agent_n == 0:
                    if (self.agents[1].pos == agent.fwd_cell_pos.pos).all():
                        ...
                        self.agents[1].foodpos = [ x for x in self.agents[1].foodpos if not (x==agent.fwd_cell_pos.pos).all()]
                        self.agents[2].foodpos = [ x for x in self.agents[2].foodpos if not (x==agent.fwd_cell_pos.pos).all()]
                        self.place_back_agent_obj(agent, **self.agent_spawn_kwargs)
                        agent.eatfood = False
                        self.place_back_agent_obj(self.agents[1], **self.agent_spawn_kwargs)

                    if (self.agents[2].pos == agent.fwd_cell_pos.pos).all():
                        self.grid.set(agent.fwd_cell_pos.pos[0],agent.fwd_cell_pos.pos[1], None)
                        agent.foodpos = [ x for x in agent.foodpos if not (x==agent.fwd_cell_pos.pos).all()]
                        self.agents[1].foodpos = [ x for x in self.agents[1].foodpos if not (x==agent.fwd_cell_pos.pos).all()]
                        self.agents[2].foodpos = [ x for x in self.agents[2].foodpos if not (x==agent.fwd_cell_pos.pos).all()]
                        self.place_back_agent_obj(agent, **self.agent_spawn_kwargs)
                        agent.eatfood = False
                        self.place_back_agent_obj(self.agents[2], **self.agent_spawn_kwargs)
