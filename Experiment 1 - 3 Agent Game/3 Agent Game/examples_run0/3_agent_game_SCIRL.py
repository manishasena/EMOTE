...
parser.add_argument('--sympathy_mode', default="true", type=str, nargs='?', help='sympathy_mode')
parser.add_argument('--IRL_type', default="SCIRL", type=str, nargs='?', help='IRL method')
...
class GradientDescent(object):
   ...
   def run( self, f_grad, f_proj=None, b_norm=False ): #grad is a function of theta
      ...
      while True:#Do...while loop
         ...
         current_norm = np.linalg.norm( DeltaTheta )
         if b_norm and  current_norm > 0.:
             DeltaTheta /= np.linalg.norm( DeltaTheta )
...
class StructuredClassifier(GradientDescent):
    ...
    def __init__(self, data, x_dim, phi, phi_dim, Y):
        ...
        self.phi=phi
        self.label_set = Y
        self.theta_0 = np.zeros((phi_dim,1))
    ...
    def greedy_policy(self, omega, phi, A, s_dim=2 ): 
        def policy( *args ):
            state_actions = [np.hstack(args+(a,)) for a in A]
            q_value = lambda sa: float(np.dot(omega.transpose(),phi(sa)))
            best_action = self.argmax( state_actions, q_value )[-1] #FIXME6: does not work for multi dimensional actions
            return best_action
        vpolicy = non_scalar_vectorize( policy, (s_dim,), (1,1) )
        return lambda state: vpolicy(state).reshape(state.shape[:-1]+(1,))

    def argmax(self, set, func ):
     return max( zip( set, map(func,set) ), key=lambda x:x[1] )[0]
    
    def structured_decision(self, theta):
        def decision( x ):
            score = lambda xy: np.dot(theta.transpose(),self.phi(xy)) + self.structure(xy)
            input_label_couples = [np.hstack([x,y]) for y in self.label_set]
            best_label = self.argmax(input_label_couples, score)[-1]
            return best_label
    ...
    def gradient(self, theta):
        ...
        phi_star = self.phi(np.hstack([self.inputs,y_star]))
        return np.mean(phi_star-self.phi_xy,axis=0)
    
    def run(self):
        f_grad = lambda theta: self.gradient(theta)
        theta = super(StructuredClassifier,self).run( f_grad, b_norm=True)
        classif_rule = self.greedy_policy(theta,self.phi,self.label_set)
...
def non_scalar_vectorize(func, input_shape, output_shape):
    ...
    def vectorized_func(arg):
        ...
        assert np.prod(arg.shape) % np.prod(input_shape) == 0, "Baaa"
        nbinputs = np.prod(arg.shape)//np.prod(input_shape)
        if nbinputs == 1:
            return func(arg)
        ...
        return np.array(answers).reshape(outer_shape+output_shape)
    return vectorized_func

class TestRLAgent:
    def __init__(self, sympathetic_mode, empathetic_mode, Ghost_model_type, loss1_weight = 0.5, bench_mark = 0, training_mode = True, load_file = None, IRL_method = None):
    ...
    def reward_SCIRL(self, name):

        # Sample trajectory
        #TRAJS = self.human_replay_memory[name].copy()
        if len(self.human_replay_memory[name]) > 500:
            TRAJS = [self.human_replay_memory[name][i] for i in range(0,500)]
        else:
            TRAJS = self.human_replay_memory[name].copy()

        obs_full = extract(TRAJS,0)
        obs = extract(obs_full,0)
        if name == 'human':
            obs_h = extract(obs,1)
            obs_h = [np.array(i)/self.pixel_scale for i in obs_h]
            obs_h = rgb2gray(obs_h).reshape(len(TRAJS),-1)

            # Extract x and y position of human
            pos = extract(obs_full,2)
            pos_x = np.array(extract(pos,0)).reshape(-1,1)
            pos_y = np.array(extract(pos,1)).reshape(-1,1)
            ...
            # Extract x and y position of human
            pos = extract(obs_full,3)
            pos_x = np.array(extract(pos,0)).reshape(-1,1)
            pos_y = np.array(extract(pos,1)).reshape(-1,1)
        
        actions = np.array(extract(TRAJS,1)).reshape(-1,1)

        state = np.concatenate((obs_h, pos_x), 1)
        state = np.concatenate((state, pos_y), 1)
        state_action_original = np.concatenate((state, actions),1)

        rewards_original = np.array(extract(TRAJS,4)).reshape(len(TRAJS),-1)
        ...
        for start_index in range(0,len(TRAJS)):
            end_index = (i for i in range(start_index,len(TRAJS)) if TRAJS[i][done_index] == True or i==len(TRAJS)-1).__next__()
            data_MC = state_action_original[start_index:end_index+1,:]
            rewards = rewards_original[start_index:end_index+1,:]
            GAMMAS = range(0,len(data_MC))
            GAMMAS = np.array([el for el in map( lambda x: pow(GAMMA,x), GAMMAS)])
            state_action = data_MC[0,:]
            state = data_MC[0,:-1]
            action = data_MC[0,-1]
            mu = None
            if len(data_MC) > 1:
                mu = np.dot( GAMMAS,np.squeeze(rewards))
            else:
                mu = np.squeeze(rewards)
            feature_expectations_MC[str(state_action)] = mu
            for other_action in [a for a in ACTION_SPACE if a != action]:
                state_action=np.hstack([state,other_action])
                feature_expectations_MC[str(state_action)]=GAMMA*mu


        # We run SCIRL again, with the MC approach for mu approximation
        sa=state_action_original
        single_mu = lambda sa:feature_expectations_MC[str(sa)]
        mu_E = non_scalar_vectorize(single_mu, (28,), (4,1))
        SCIRL_MC = StructuredClassifier(sa, 27, mu_E, 4, ACTION_SPACE)
        void,theta_SCIRL_MC = SCIRL_MC.run()
        #SCIRL_reward_MC = lambda sas:np.dot(theta_SCIRL_MC.transpose(),psi(sas[:1]))[0]
        #vSCIRL_reward_MC = non_scalar_vectorize( SCIRL_reward_MC, (5,),(1,1) )

        # Update rewards
        self.other_rewards[name] = theta_SCIRL_MC.reshape(4,)
    ...
    def end_episode(self):
        ...
        # Update reward prediction of other agents
        if self.IRL_method == 'SCIRL':
            self.reward_SCIRL('human')
            self.reward_SCIRL('adversary')
...
agents = TestRLAgent(sympathetic_mode, empathetic_mode, Ghost_model_type, loss1_weight = loss1_weight, bench_mark = bench_mark, training_mode = training_mode, IRL_method = IRL_method)
...
for loop in loops:
    ...
    if training_mode:
        ...
        # Create a human player interface per the class defined above
        agents = TestRLAgent(sympathetic_mode, empathetic_mode, Ghost_model_type, loss1_weight = loss1_weight, bench_mark = bench_mark, training_mode = training_mode, load_file = str(loop), IRL_method = IRL_method)
