...
parser.add_argument('--IRL_type', default="RE", type=str, nargs='?', help='IRL method')
...
class RelativeEntropy(GradientDescent):
    ...
    def __init__(self, mu_E, mus):
        self.theta_0 = np.zeros(mu_E.shape)
        self.Mu_E = mu_E
        self.Mus = mus
    ...
    def gradient(self, theta):
        ...
        denominator = 0
        for mu in self.Mus:
            c = np.exp(np.dot(theta.transpose(),mu))
            numerator += c*mu
            denominator += c
        assert denominator != 0,"A sum of exp(...) is null, some black magic happened here."
        return self.Mu_E - numerator/denominator - np.sign(theta)*self.Epsilon
...
class TestRLAgent:
    def __init__(self, sympathetic_mode, empathetic_mode, Ghost_model_type, loss1_weight = 0.5, bench_mark = 0, training_mode = True, load_file = None, IRL_method = None):
        ...
        # Load random trajectories of human and adversary prepared earlier
        self.data_r_RE_file = {}
        self.data_r_RE_file['human'] = pickle.load(open('IA1' + '.pkl', 'rb'))
        self.data_r_RE_file['adversary'] = pickle.load(open('IA2' + '.pkl', 'rb'))

        self.save_ready = {}
        self.save_ready['human'] = 0
        self.save_ready['adversary'] = 0

        self.save_done = {}
        self.save_done['human'] = True
        self.save_done['adversary'] = True
    ...
    def save_other_step(self, obs, act, nextobs, done, reward_vector, door, door_ns, name):
        ...
        if name == 'adversary': #'human'

            if done:
                self.save_ready[name] += 1

            if (len(self.human_replay_memory[name]) > 1000) and (not self.save_done[name]) and (self.save_ready[name] > 5):
                #Save to text file
                print('saved: ' + name)
                pickle.dump(self.human_replay_memory[name], open(name+'.pkl', 'wb'))

                self.save_done[name] = True
                print(len(self.human_replay_memory[name]))
    ...
    def reward_RE(self, name):

        def phi(reward, action):
            Mu_E = np.zeros(((4)*5,1))
            Mu_E[action[0]*(4):(action[0]+1)*4] = reward.reshape(4,1)

            return Mu_E

        data_r_RE = self.data_r_RE_file[name]
        ...
        actions_RE = np.array(extract(data_r_RE,1)).reshape(-1,1)
        ...
        rewards_RE = np.array(extract(data_r_RE,4)).reshape(len(data_r_RE),-1)
        ...
        for i in range(0,len(TRAJS)):
            #Mu_E += pow(GAMMA,t)*phi(rewards_original[i],actions[i])
            Mu_E += pow(GAMMA,t)*rewards_original[i].reshape(4,1)
            if TRAJS[i][done_index]:
                t=0.
            else:
                t+=1.
        ...
        for i in range(0,len(data_r_RE)):
            #mu += pow(GAMMA,t)*phi(rewards_RE[i],actions_RE[i])
            mu += pow(GAMMA,t)*rewards_RE[i].reshape(4,1)
            if data_r_RE[i][done_index]:
                mu /= t+1.
                Mus_RE.append(mu)
                t=0.
                mu = np.zeros((4,1))
        ...
        RE_LSPI = RelativeEntropy(Mu_E, Mus_RE)
        ...
        # Update rewards
        self.other_rewards[name] = theta_RE_LSPI.reshape(4,)
    ...
    def end_episode(self):
        ...
        if self.IRL_method == 'RE':
            self.reward_RE('human')
            self.reward_RE('adversary')
    ...
    for episodes in episode_range:
        ...
        while not done:
            ...
            for agent_no in [0,1,2]:
                ...
                # Uncomment this code just to create random trajectory for RE
                #if (len(env.agents[1 or 2].foodpos)==0):
                #    done = True
                #    won = True
