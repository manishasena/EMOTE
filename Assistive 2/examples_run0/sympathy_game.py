    ...
    for episodes in episode_range:
        ...
        if agents.training_mode:
            ...
            if agents.sympathetic_mode:
                ...
                reward_symp_array = [float(agents.reward_foodh)/float(agents.beta_foodh_cnt), float(agents.reward_food)/float(agents.beta_food_cnt),float(agents.reward_door)/float(agents.beta_door_cnt),float(agents.reward_win)/float(agents.beta_win_cnt)]
                log_file.write("# %4d | reward: %r \n" %
            ...
            if agents.sympathetic_mode:
                ...
                reward_symp_array = [float(agents.reward_foodh_pnext)/float(agents.beta_foodh_cnt), float(agents.reward_food_pnext)/float(agents.beta_food_cnt),float(agents.reward_door_pnext)/float(agents.beta_door_cnt),float(agents.reward_win_pnext)/float(agents.beta_win_cnt)]
                log_file.write("# %4d | reward: %r \n" %
                                            (episodes,list(reward_symp_array)))
